 ...
 * 面板 310x148，晴空深色主题，与更新窗口同一套控件与配色：
 * 左侧上下两个等宽输入框，右侧结果区两行居中显示（连接/保存 结果），底部三个等大按钮。
 *
 * 功能：
...
public class ServerSettingsScreen extends Screen {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // 顶部安全提示
        g.drawString(font, "请只连接可信任的服务器", x0, panelY + 12, ModStyle.ORANGE);
        ModStyle.drawHairline(g, x0, panelX + PANEL_W - 20, panelY + 26);

        g.drawString(font, "服务器地址", x0, panelY + 38, ModStyle.TEXT_DIM);
        g.drawString(font, "端口", x0, panelY + 70, ModStyle.TEXT_DIM);
        ...
        if (rw > 20) {
            ...
            if (!resultLine1.isEmpty()) {
                g.drawCenteredString(font, trimToWidth(font, resultLine1, rw), rcx, panelY + 60, resultColor1);
            }
            if (!resultLine2.isEmpty()) {
                g.drawCenteredString(font, trimToWidth(font, resultLine2, rw), rcx, panelY + 76, resultColor2);
            }
        }
    ...
    private void drawInputBg(GuiGraphics g, EditBox box) {
    ...
    private void drawPlaceholder(GuiGraphics g, Font font, EditBox box, String text) {
        g.drawString(font, text, box.getX() + 4, box.getY() + (box.getHeight() - 8) / 2,
                ModStyle.TEXT_DISABLED);
    }
